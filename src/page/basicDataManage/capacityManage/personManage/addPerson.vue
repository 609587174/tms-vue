  ...
  methods: {
    returnToPage: function() {
      if (this.$route.query.operate === 'edit') {
        this.$router.push({ path: `/basicDataManage/capacityManage/personManage/personDetail/${this.$route.query.id }/` });
      } else {
          ...
          this.$$http(apiName, postData).then((results) => {
            ...
            if (results.data && results.data.code == 0 && results.data.data) {
              ...
              if (isReview) {
                ...
                this.$router.push({ path: "/basicDataManage/capacityManage/personManage/addPerson", query: { activeStep: stepNum - 1, id: id, operate: operate } });
