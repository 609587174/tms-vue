  ...
  methods: {
    ...
    returnToPage: function() {
      if (this.$route.query.id) {
        this.$router.push({ path: `/basicDataManage/shipperManage/shipperDetail/${this.$route.query.id}` });
      } else {
        this.$router.push({ path: "/basicDataManage/shipperManage/shipperManageList" });
      }
    },
        ...
        if (valid) {
          ...
          this.$$http(apiName, postData).then((results) => {
            ...
            if (results.data && results.data.code == 0 && results.data.data) {
              ...
              if (isReview) {
                if (this.$route.query.id) {
                  this.$router.push({ path: `/basicDataManage/shipperManage/shipperDetail/${results.data.data.id}` });
                } else {
                  this.$router.push({ path: "/basicDataManage/shipperManage/shipperManageList" });
                }
              } else {
                let id = results.data.data.id;
                this.$router.push({ path: "/basicDataManage/shipperManage/editShipper", query: { activeStep: stepNum - 1, id: id } });
              }
            }
