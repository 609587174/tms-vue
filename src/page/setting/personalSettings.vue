            ...
            <div class="basic-data" v-if="editStatus.basicData">
              ...
              <div class="basic-data-list">
                <el-form-item label="部门:">
                  <div>{{users.department.group_name}}<span class="text-secondary ml-30" v-if="!users.is_leader">（修改请联系管理员）</span></div>
                </el-form-item>
              </div>
              <div class="basic-data-list">
                <el-form-item label="职位:">
                  <div>{{users.carrier_role.role_name}}<span class="text-secondary ml-30" v-if="!users.is_leader">（修改请联系管理员）</span></div>
                </el-form-item>
              </div>
              <div class="basic-data-list">
                <el-form-item label="">
              ...
              <div class="basic-data-list">
                <el-form-item label="部门:" prop="department">
                  <el-select v-model="editMsgForm.department" placeholder="请选择" v-loading="departmentLoading" v-if="users.is_leader">
                    <el-option v-for="(item,key) in departmentData" :key="item.id" :label="item.group_name" :value="item.id"></el-option>
                  </el-select>
                  <div v-else>{{users.department.group_name}}<span class="text-secondary ml-30">（修改请联系管理员）</span></div>
                </el-form-item>
              </div>
              <div class="basic-data-list">
                <el-form-item label="职位:" prop="position">
                  <el-select v-model="editMsgForm.position" placeholder="请选择" v-loading="positionLoading" v-if="users.is_leader">
                    <el-option v-for="(item,key) in positionData" :key="item.id" :label="item.role_name" :value="item.id"></el-option>
                  </el-select>
                  <div v-else>{{users.carrier_role.role_name}}<span class="text-secondary ml-30">（修改请联系管理员）</span></div>
                </el-form-item>
              </div>
              <div class="basic-data-list">
...
<script type="text/javascript">
  export default {
    ...
    created() {
      this.users = this.pbFunc.getLocalData('users',true);
    },
    methods: {
      ...
      updateBtn(type,status){
        this.editMsgForm = {
          mobile_phone:this.users.phone,
          email:this.users.email,
          username:this.users.username,
          nick_name:this.users.nick_name,
          position:this.users.carrier_role.id,
          department:this.users.department.id,
          old_pwd: '',
          new_pwd: '',
          confirm_pwd:'',
        }
      ...
      getDepartmentList: function() {
        ...
        this.$$http('getDepartmentList', { pagination: false }).then((results) => {
          if (results.data && results.data.code == 0) {
            this.departmentData = results.data.data;
      ...
      getPositionList: function(departmentId) {
        ...
        this.$$http('getPositionList', postData).then((results) => {
          ...
          if (results.data && results.data.code == 0) {
            this.positionData = results.data.data.results;
          }
        }).catch((err) => {
